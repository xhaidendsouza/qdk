...
impl Lowerer {
    ...
    pub fn push_semantic_error(&mut self, kind: SemanticErrorKind) {
        let kind = crate::error::ErrorKind::Semantic(crate::semantic::Error(kind));
    ...
    pub fn push_const_eval_error(&mut self, kind: ConstEvalError) {
        let kind = crate::error::ErrorKind::ConstEval(kind);
        let error = self.create_err(kind);
        self.errors.push(error);
    }

    /// Creates an error from the given kind with the current source mapping.
    fn create_err(&self, kind: crate::error::ErrorKind) -> WithSource<crate::error::Error> {
        let error = crate::error::Error(kind);
